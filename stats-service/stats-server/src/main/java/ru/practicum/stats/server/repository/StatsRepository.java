...
public interface StatsRepository extends JpaRepository<Event, Long> {
    ...
    @Query(value = "SELECT new ru.practicum.stats.service.server.model.Statistic(app, uri, COUNT(ip)) \n" +
            "FROM Event \n" +
            "WHERE uri IN (:uris) \n" +
    ...
    @Query(value = "SELECT new ru.practicum.stats.service.server.model.Statistic(app, uri, COUNT(DISTINCT(ip))) \n" +
            "FROM Event \n" +
            "WHERE uri IN (:uris) \n" +
            "  AND created BETWEEN :start \n" +
            "                  AND :end \n" +
